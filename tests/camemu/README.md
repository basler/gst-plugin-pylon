...
| Device selection | serial, index, ambiguous-device error |
...
| Buffers | PyGObject appsink count (optional) |

Gstcheck unit tests in `tests/check/pylon/pylonsrc_camemu.c` cover element
creation, state transitions, serial property, and ambiguous-device failure.
Buffer and format coverage is in the shell suite above.
