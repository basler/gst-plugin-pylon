#!/usr/bin/env bash
# Functional test suite for pylonsrc using Basler camera emulators (PYLON_CAMEMU).
# Requires a built plugin and PYLON_ROOT pointing at pylon 26.x.
...
expect_output "plugin_reports_pylon_sdk" "pylon SDK" \
  gst-inspect-1.0 pylonsrc

expect_output "inspect_has_device_serial_property" "device-serial-number" \
  gst-inspect-1.0 pylonsrc

expect_output "inspect_has_cam_child_proxy" "cam" \
  gst-inspect-1.0 pylonsrc

expect_output "inspect_has_stream_child_proxy" "stream" \
  gst-inspect-1.0 pylonsrc

expect_output "inspect_lists_gray8_caps" "GRAY8" \
  gst-inspect-1.0 pylonsrc

expect_output "inspect_lists_bayer_caps" "video/x-bayer" \
  gst-inspect-1.0 pylonsrc

# Second inspect after cache warm-up
expect_output "inspect_works_after_cache_warmup" "Basler/Pylon source element" \
  gst-inspect-1.0 pylonsrc
...
# --- pipeline lifecycle ---
expect_ok "pipeline_null_to_playing_to_eos" \
  gst_pipeline pylonsrc device-serial-number="$EMU_SERIAL_0" num-buffers=15 ! fakesink
...
# --- repeated open/close (regression for SDK lifetime) ---
